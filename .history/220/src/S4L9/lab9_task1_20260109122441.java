...
public class lab9_task1 {
    ...
    public static void laughTale(Integer[] warLords, Edge[] adjL, int sSt, int sP, int lSt, int lP){
        int S_S = sSt, L_S =lSt; boolean SR = false, LR = false;
        while(sP>0 && SS){
            if(S_S == 10){SR = true; break;}
            if(sP>(warLords[S_S]+adjL[S_S].weight)){ sP-= (warLords[S_S]+adjL[S_S].weight);S_S= adjL[S_S].toV;}
        }
