...
public class lab9_task1 {
    ...
    public static void laughTale(Integer[] warLords, Edge[] adjL, int sSt, int sP, int lSt, int lP){
        int SS = sSt, LS =lSt; boolean SR = false, LR = false;
        while(sP>(warLords[SS]+adjL[SS].weight) && SS<10){
            if(sP>){ sP-= (warLords[SS]+adjL[SS].weight);SS= adjL[SS].toV;}
        }
        while(lP>(warLords[LS]+adjL[LS].weight) && LS<10){
            lP-= (warLords[LS]+adjL[LS].weight);LS= adjL[LS].toV;
            System.out.println(lP+"  "+LS);
        }
        if(SS!=10 && LS!=10){System.out.println("No Winner");return;}
        if(SS==10 && LS == 10){
            if (sP>lP) {
                System.out.println("Shanks Wins");return;
            }System.out.println("Luffy Wins");return;
        }
        if(SS == 10){System.out.println("Shanks Wins");return;} System.out.println("Luffy Wins");
