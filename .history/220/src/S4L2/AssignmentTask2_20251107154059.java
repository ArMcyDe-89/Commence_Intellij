    ...
    public static Node organizeBooks(Node head, Integer[] popularity) {
        ...
        for(int i = 0;i<popularity.length;i++){
            if(popularity[i]>max){
                ...
                }Node M = N.next; N.next = N.next.next; M.next = head; head = M; N =head; i =0;
            }
            else if(popularity[i] == max && i>0){
                popularity[i] = popularity[1]; popularity[1] = max; int pos = 0;
                while(pos < i-1){
                    N = N.next; System.out.println(N.elem);
                    pos++; System.out.println(pos);
                }Node M = N.next; N.next = N.next.next; M.next = head.next;head.next = M; N=head;i=1; 
