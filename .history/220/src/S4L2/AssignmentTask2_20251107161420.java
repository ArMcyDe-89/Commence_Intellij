    ...
    public static Node organizeBooks(Node head, Integer[] popularity) {
        ...
        for(int i = 0;i<popularity.length;i++){
            if(popularity[i]>max){
                ...
                while(pos<i-1){
                    N=N.next; System.out.println("N.elem - "+N.elem);
                    pos++; System.out.println("pos - "+pos);
                }
                Node M = N.next;
                N.next = N.next.next;
                M.next = head;
                ...
                for(int j = i; j<popularity.length;j++){
                    if(popularity[i]<popularity[j]){
                        ...
                        while (pos<j-1){
                            N = N.next;
                            pos++;
                        } Node sec = N; Node latsec = sec.next.next; Node change = sec.next; sec.next = prev.next; sec.next.next = latsec;change.next =latprev prev.next = change; 
                    }
                }
            }
        }
