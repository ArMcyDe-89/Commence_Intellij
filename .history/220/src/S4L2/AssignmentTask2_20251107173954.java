    ...
    public static Node organizeBooks(Node head, Integer[] popularity) {
        ...
        for(int i =0;i<popularity.length-1;i++){
            if (i==0 && its ==0){
                ...
                Node M = N.next; N.next = N.next.next; M.next = N; head = M; N = head; LinkedList.printLL(head);System.out.println(Arrays.toString(popularity));
            }
            else if (popularity[i]<popularity[i+1]){
                if (i==0 && its >0){
                int X = popularity[i]; popularity[i] = popularity[i+1]; popularity[i+1] = X;
                Node M = N.next; N.next = N.next.next; M.next = N; head = M; N = head; LinkedList.printLL(head);System.out.println(Arrays.toString(popularity)); continue;
            }
                ...
                Node M = N.next; Node J = N.next.next; M.next = J.next; J.next = M; N.next = J; N = head; i=0;LinkedList.printLL(head);System.out.println(Arrays.toString(popularity));
            } its++;
        }its = 0;
