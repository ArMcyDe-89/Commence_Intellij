...
public class AssignmentTask4{
    ...
    public static Node idGenerator(Node head1, Node head2, Node head3) {
        Node N = head1, M = head2, O = head3;
        Node ID =  new Node(null); Node T =  ID; 
        while (N!=null) {
            Node enter = new Node(N.elem); enter.next = ID; ID = enter; N=N.next;
        }
        while (T.next != null) {
            T=T.next;
        }
        while (M!=null && O!=null) {
            T.next = new Node((int)M.elem + (int)O.elem); T = T.next; M =M.next; O = O.next;
        }
        ...
        return ID; // Remove this when you're ready to return the new head
    }
