...
public class AssignmentTask2{
    ...
    public static Node organizeBooks(Node head, Integer[] popularity) {
        ...
        for(int i = 0;i<popularity.length;i++){
            if(popularity[i]>max){
                max =popularity[i]; popularity[i] = popularity[0]; popularity [0] = max; int pos = 0; System.out.println(i);
                while(pos<i-1){
                    N=N.next;
                    pos++;LinkedList.printLL(N);System.out.println(i);
                }Node M = N.next; N.next = N.next.next; M.next = head; head = M; N =head; i =0;
            }
            else if(popularity[i] == max){
                popularity[i] = popularity[1]; popularity[1] = max; int pos = 0;LinkedList.printLL(N);System.out.println(i);
                while(pos < i-1){
                    N = N.next;
                    pos++;
                }Node M = N.next; N.next = N.next.next; M.next = head.next;head.next = M; N=head;i=0; 
