...
public class Runner {
    public static void main(String[] args) {
        ...
        if(k>nums.length){while(true){System.out.print("Please enter a number smaller than the length of nums :"); k =sc.nextInt();if(k<=nums.length){break;;} System.out.println();}}
        Integer [] KK = new Integer[k];
        MaxHeap max = new MaxHeap(nums.length);
        for(int i = 0;i<nums.length;i++){max.insert(nums[i]);}
        for(int i=0; i<k;i++){nums[i]=max.extractMax();}
        System.out.println("The biggest elemnts are:"); printArray(KK);
