public class MinHeap {
    ...
    public void sink(Integer[] heap,int i, int size){
        if ((2*i<size)&&(2*i+1<size) && heap[2*i]<heap[2*i+1] && heap[2*i]<heap[i]) {
            int swap = heap[i]; heap[i] = heap[2*i];heap[2*i]=swap; sink(heap,2*i, size);
        }
        if ((2*i+1<size) && heap[2*i]>heap[2*i+1] && heap[2*i+1]<heap[i]) {
            int swap = heap[i]; heap[i] = heap[2*i+1];heap[2*i+1]=swap; sink(heap,2*i+1,size);
        }
