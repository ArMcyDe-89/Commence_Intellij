...
public class HashTable {
    ...
    private int hashFunction( String key ){
        ...
        System.out.println(sum+"  sum and mod  "+sum%length);
        // TO DO 
        return (sum%length);
    }
    ...
    public void insert(String key, Integer value){
        int index = hashFunction(key); System.out.println((int)'u'+"  "+index+"   "+(int)'o');
        if (this.ht[index%this.ht.length] == null) {
            this.ht[index%this.ht.length] = new FruitNode(key,value); return;
        }
        FruitNode N = this.ht[index];
        while (N.next!=null && (Integer)N.next.fruit[1]>value) {
            N=N.next;
