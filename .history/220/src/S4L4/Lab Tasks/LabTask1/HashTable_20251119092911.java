...
public class HashTable {
    ...
    public void insert(String key, Integer value){
        ...
        if (this.ht[index] == null) {
            ...
            // }N = this.ht[index];
            //     while(N.next!=null ){
            //         if((Integer)N.fruit[1]>value ){return;}
            //         N=N.next;
            //     }
            while(N.next!=null){
                if(((String)N.fruit[0]).equals(key)){
                ...
                N=N.next;
            }FruitNode m = new FruitNode(key, value); m.next = N.next;N.next = m;
            while(N.next!=null){
                // if((Integer)N.fruit[1]<(Integer)N.next.fruit[1]){
                //     FruitNode M = N; N = N.next; M.next = N.next; N.next = M;
                // }
                if (N.next.next!=null && (Integer)N.next.fruit[1]<(Integer)N.next.next.fruit[1]) {
                    FruitNode M =N.next.next; N.next.next = N.next.next.next; M.next = N.next.next; N.next.next = M;
                }
                N=N.next;
            }
        }
