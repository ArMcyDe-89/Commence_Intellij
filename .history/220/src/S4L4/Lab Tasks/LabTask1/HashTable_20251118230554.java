...
public class HashTable {
    ...
    private int hashFunction( String key ){
        ...
        if(length%2 == 0){
            for(int i =0;i<key.length() && i%2 == 0;i++){
                    sum += (int)key.charAt(i); System.out.println((int)key.charAt(i));
            }
        }
        else{
            for(int i =0;i<key.length() && i%2 != 0;i++){
                    sum += (int)key.charAt(i); System.out.println(key+(int)key.charAt(i));
            }
        }
    ...
    public void insert(String key, Integer value){
        ...
        if (this.ht[index] == null) {
            ...
            }N = this.ht[index]; System.out.println(N.fruit[0]);
            if((Integer)N.fruit[1]>value && N.next !=null){
                while((Integer)N.next.fruit[1]>value ){
                    N=N.next;
                }
            }FruitNode m = new FruitNode(key, value); m.next = N.next;
            N.next = m;
        }
        // TO DO
    }
