...
public class HashTable {
    ...
    private int hashFunction( String key ){
        if(key.length()%2!=0){key+="N";}
        String S = ""; int sum = 0,count = 0;
        for(int i =0; i<key.length();i+=2){
            S+=String.valueOf((int)key.charAt(i))+String.valueOf((int)key.charAt(i+1));
            sum+=Integer.parseInt(S); S="";count++;
        }
        // TO DO !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        return sum%key.length(); //remove this line
    }
    ...
    public String searchHashtable( Object[] keyValuePair ){
        // TO DO !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        int H = hashFunction((String)keyValuePair[0]);
        PairNode N = this.ht[H];
        while (N!=null) {
            if(N.key.equals(keyValuePair[0])){return "Found";}
            N=N.next;
        }
