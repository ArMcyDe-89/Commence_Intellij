...
public class HashTable {
    ...
    public void remove( Integer key ){
        PairNode N = this.ht[hashFunction(key)];
        if ((int)N.key== (int)key){System.out.println(N.key+"  "+key); N = N.next; this.ht[hashFunction(key)] = N;return;}
            while (N.next != null) {
                if (N.next.key.equals(N)key){System.out.println(N.key+"  "+key); N.next = N.next.next; this.ht[hashFunction(key)] = N;return;}
                N = N.next;
            }
        // TO DO 
    }
