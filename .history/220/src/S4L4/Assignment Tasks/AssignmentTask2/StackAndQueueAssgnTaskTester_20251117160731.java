...
public class StackAndQueueAssgnTaskTester {
    ...
    public static void evalMathExpression(String expression) {
        ...
        if (expression.charAt(i) == '(' || expression.charAt(i) == '{' || expression.charAt(i) == '[') {
            while(expression.charAt(j) != ')' || expression.charAt(j) == '}' || expression.charAt(i) == ']'){
                if(Integer.parseInt(expression.charAt(j)))
                j++;
            }
        }
            while(expression.charAt(j)!= ' ' && expression.charAt(j)<='9' && expression.charAt(j)>= '0'){
                add+= expression.charAt(j); j++;
            }S.push(add); Q.enqueue(add);
    ...
    public static int Operation (Stack S, LinkedListQueue Q, char op) {
        int x = Integer.parseInt((String)Q.dequeue()); int y = Integer.parseInt((String)Q.dequeue());
        if(op == '^'){return (int)Math.pow(x, y);}
        else if(op == '*'){return x*y;} else if(op == '/'){return x/y;}
