    ...
    public static void evalMathExpression(String expression) {
        ...
        for(int i = 0; i<expression.length();i++){
            if(expression.charAt(i) == '(' || expression.charAt(i) == '{' || expression.charAt(i) == '['){
                ...
                if(S.peek()==null){System.out.println("Invalid Expression"); return;}
                if ('(' == (char)S.peek()){
                    S.pop();
                }
            }
        ...
        for(int  i = 0;i<expression.length();i++){
            if(expression.charAt(i) == '/'){
                sum += ((int)expression.charAt(i-1) - (int)'0') / ((int)expression.charAt(i+1) - (int)'0');System.out.println(expression.charAt(i-1)+"   "+(int)expression.charAt(i+1));
            }
            else if(expression.charAt(i) == '^'){
                sum += Math.pow(((int)expression.charAt(i-1) - (int)'0'),((int)expression.charAt(i+1) - (int)'0'));System.out.println(expression.charAt(i-1)+"   "+(int)expression.charAt(i+1));
            }
            else if(expression.charAt(i) == '*'){
                sum += ((int)expression.charAt(i-1) - (int)'0') * ((int)expression.charAt(i+1) - (int)'0');System.out.println(expression.charAt(i-1)+"   "+(int)expression.charAt(i+1));
            }
