    ...
    public static void evalMathExpression(String expression) {
        ...
        // } Q.displayQueue();
            while(j<expression.length() && expression.charAt(j)!= ' ' ){
                if(expression.charAt(j) == '(' || expression.charAt(j) == '{' || expression.charAt(j) == '[' || expression.charAt(j) == ')' || expression.charAt(j) == '}' || expression.charAt(i) == ']'){j++;continue}
                if(c == '^'|| c == '*' || c == '/' || c == '+' || c == '-'){OpStack(S, c, sum);}
                add+= expression.charAt(j); i=++j;
            }if(add!=""){sum+=" "+add; Q.enqueue(add);}
        } Q.displayQueue();
