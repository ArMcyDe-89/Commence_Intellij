    ...
    public static void evalMathExpression(String expression) {
        ...
        for(int i = 0; i<expression.length();i++){
            ...
            if (S.isEmpty() == true && (c == '^' || c == '*' || c =='/' || c == '+'||c== '-')){S.push(c);}
                ...
                int pow = 0;
                while(power<=pow){
                 char CH = (char)S.peek(); if(CH == '^'){pow = 3;} else if (CH == '*' || CH =='/'){pow=2;} else if(CH == '+'||CH== '-'){pow = 1;}Q.enqueue(S.pop());} S.push(c);
            }
            if(c =='(' || c =='{' || c =='['){S.push(c); continue;}
            else if(c ==')' || c =='}' || c ==']'){ char CH = (char)S.peek();
                while(S.isEmpty() == false && CH !='(' && CH !='{' && CH !='['){
                    Q.enqueue(S.pop());
                } S.pop(); continue;
            }
        }
