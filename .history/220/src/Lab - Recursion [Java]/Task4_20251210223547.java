public class Task4 {
    ...
    public static Node task4B_recursive(Node head) { Node N,M;
        if (head.next == null) {
            return head;
        }
        else{
                Node rest = task4B_recursive(head)(head.next);

    // Link the current node after the reversed rest
    head.next.next = head;
    head.next = null;

    return rest;
            // N = head; 
            // M = task4B_recursive(head.next); Node save = M;
            // while(M.next!=null){M=M.next;}N.next = M.next;M.next=N; N=save; return N;
        }
        // TODO: Implement this recursively
