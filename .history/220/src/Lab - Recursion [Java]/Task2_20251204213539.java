...
public class Task2 {
    ...
    public static void task2B_recursive(int[] arr, int i) {
        if(arr.length-1-i == 0){System.out.println(arr[arr.length-1-i]);return;}
        task2B_recursive(arr, ++i); System.out.println(arr[arr.length-1-(--i)]);
    ...
    public static int task2D_recursive(int[] arr, int i) {
        int sum = 0;
        if(i == arr.length-1){return sum+=arr[i]; }
        // TODO: Implement this recursively
        return sum+=arr[i]+task2D_recursive(arr, ++i);
    }
    ...
    public static int task2F_recursive(int[] arr) {
        int multi = 1, sum =0;
         sum+=Recursive_Add(arr,0); System.out.println(sum);
         multi*=Recursive_Multi(arr,1); System.out.println(multi);
        // TODO: Implement this recursively
        return multi - sum;
    }
    public static int Recursive_Multi(int [] arr, int a) {
        int multi = 1;
        if(a<arr.length && arr[a]%2 == 0){
            multi *= Recursive_Multi(arr, ++a);
        }
        else{
            return multi *= Recursive_Multi(arr, a);
        }
        if(a<arr.length){multi*=arr[a]*Recursive_Multi(arr, a+2);}
        return multi;
    }
    public static int Recursive_Add(int []arr, int a) {
        int sum = 0;
        if(a<arr.length){sum+=arr[a]+Recursive_Add(arr, a+2);}
        return sum;
    }
    //DRIVER CODE
