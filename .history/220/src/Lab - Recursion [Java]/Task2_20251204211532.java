...
public class Task2 {
    ...
    public static int task2F_recursive(int[] arr) {
        int multi = 1, sum =0, i=0;
        while (i<arr.length) {
            if(i%2 == 0){
                sum+=Recursive_Add(arr[i++]);
            }
            else{multi*=Recursive_Multi(arr[i++]);}
        }
        // TODO: Implement this recursively
        return multi - sum;
    }
    public static int Recursive_Multi(int [] arr, int a) {
        int multi = 1;
        if(a<arr.length){multi*=Recursive_Multi(arr, a+2);}
        return multi;
    }
    public static int Recursive_Add(int []arr, int a) {
        int sum = 0;
        if(a<arr.length){sum+=Recursive_Add(arr, a+2)}
        sum+=a; return sum;
    }
    //DRIVER CODE
