public class MaxVer {
    ...
    public Integer task_2b_LIST(Node [] arr) { //same in task 3b
        ...
        public Integer task_3a_MAT(Integer [][] arr) { //same in task 3a
        int vertex = 0, Max = 0;
        for(int i =0; i< arr.length; i++){ int sum = 0;
            for(int j = 0; j<arr[i].length; j++){
                if(arr[i][j] != 0){sum+=arr[i][j];}
    ...
    public Integer task_3b_LIST(Node [] arr) { //same in task 3b
        int Max = 0, vertex = 0; Node MV = null;
        for(int i =0; i<arr.length; i++){
            Node N = arr[i]; int sum =0;
            while(N!=null){sum+=N.weight;N=N.next;}
