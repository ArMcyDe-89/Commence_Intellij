...
// ==========================================
// YOUR CLASS (With the infinite loop fix applied)
// ==========================================
class MaxVer {
    public Integer MaxDeg(Integer [][] arr) {
        int vertex = 0, Max = 0;
    ...
    // NOTE: Assuming Node has 'next' field.
    public Integer MaxDeg(Node [] arr) {
        int Max = 0, vertex = 0;
        for(int i =0; i< arr.length; i++){
            Node N = arr[i]; int count = 0;
    ...
    // NOTE: Assuming Node has 'weight' field.
    public Integer task_2b_LIST(Node [] arr) { 
        int Max = 0, vertex = 0; Node MV = null;
        for(int i =0; i<arr.length; i++){
            Node N = arr[i]; int sum =0;
    ...
    public Integer task_3a_MAT(Integer [][] arr) { 
        // Logic is identical to 2a
        return task_2a_MAT(arr);        
    }

    public Integer task_3b_LIST(Node [] arr) { 
        // Logic is identical to 2b
        return task_2b_LIST(arr);       
    }

    public void GraphConvert_arr(Integer [][] arr) {
        for(int i =0; i<arr.length;i++){
            // FIX APPLIED HERE: changed 'i<arr[i].length' to 'j<arr[i].length'
            for(int j = i+1; j<arr[i].length;j++){
                int sum = arr[i][j]+arr[j][i];
                arr[i][j] = sum; arr[j][i] =sum;
            }
...
// Used in your MaxDeg/Task2b methods
class Node {
    int weight;
    Node next;
    // Constructor if needed
    public Node(int weight) { this.weight = weight; }
}

// Used in your GraphConvert methods
class EdgeNode {
    int toV;
    int weight;
    EdgeNode next;
