...
public class QuizTwoC {
    ...
    public Integer findMax(BTNode root, int max){
        if(root == null){ return 0;} if(root.left == null &&  root.right == null){return (int)root.elem;}
        if(root.right!= null){ if((int)root.right.elem>max){max = (int)root.right.elem;}return findMax(root, max);}
        if(root.left != null ){if((int)root.right.elem>max){max = (int)root.right.elem;}return findMax(root, max);}
