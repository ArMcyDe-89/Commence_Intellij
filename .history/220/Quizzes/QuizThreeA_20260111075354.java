public class QuizThreeA {
    ...
    public static void main(String[] args) {
        // --- Test Case 1 ---
        // S = {'m','i','s','i','s','i','s','i','p','p','i'}
        // Frequencies: m=1, i=4, s=4, p=2. Top 2: 4, 4
        char[] s1 = {'m','i','s','i','s','i','s','i','p','p','i'};
        int k1 = 2;
        System.out.println("Test Case 1 Output:");
        topKFreqLetters(s1, k1); 
        System.out.println("\nExpected: 4 4");
        ...
        // --- Test Case 2 ---
        // S = {'a','a','a','c','b','b','c','b','a'}
        // Frequencies: a=4, b=3, c=2. Top 2: 4, 3
        char[] s2 = {'a','a','a','c','b','b','c','b','a'};
        int k2 = 2;
        System.out.println("Test Case 2 Output:");
        topKFreqLetters(s2, k2);
        System.out.println("\nExpected: 4 3");
    }
    static void topKFreqLetters(char []S, int K){
        char [] alp = new char[26];
        for(int i=0;i<S.length;i++){
            alp[S[i]-'a']+;
        }
        MaxHeap max = new MaxHeap(S.length);
        for(int i=0; i<alp.length;i++){
            if(alp[i]>0){
                max.insert(alp[i]);
            }
        }
        for(int i=0;i<K;i++){System.out.println( max.extractMax());}
    }
