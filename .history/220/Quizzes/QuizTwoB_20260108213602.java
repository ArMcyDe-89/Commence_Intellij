public class QuizTwoB {
    ...
    public Integer faulty(BTNode root, Integer D){
        if(root == null){ return 0; }
        // Base case: Leaf nodes are never faulty
        if(root.left == null && root.right == null){ return 0; }
        
        int count = 0;
        
        // Check if CURRENT node is faulty based on LEFT child
        boolean leftFaulty = (root.left != null && 
                             (Math.abs((int)root.elem - (int)root.left.elem) > D));
                             
        // Check if CURRENT node is faulty based on RIGHT child
        boolean rightFaulty = (root.right != null && 
                              (Math.abs((int)root.elem - (int)root.right.elem) > D));

        // If either condition is true, this node is faulty
        if (leftFaulty || rightFaulty) {
            count++;
        }
