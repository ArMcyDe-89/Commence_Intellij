...
public class QuizTwoC {
    ...
    public Integer sum_subtree_max(BTNode root){
        int sum=0, sub = findMax(root.right, Integer.MIN_VALUE) - findMax(root.left, Integer.MIN_VALUE);

    }

    public Integer helper(BTNode root, int sub){
        if((int)root.elem>sub){return (int)root.elem+helper(root.left, sub)+helper(root, sub)}
    }
