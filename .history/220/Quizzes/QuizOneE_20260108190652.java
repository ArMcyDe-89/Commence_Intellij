public class QuizOneE {
    public void minInMid(Node head){
        ...
        // while (N != null) { // this swaps the node at the middle
        ...
        while (N!=null) { // this inserts the node at the middle
            if(N.elem<M.elem){M=N;}
            N=N.next;
        }
        M.prev.next = M.next;
        if (M.next != null) {  // CRITICAL CHECK for NullPointerException
            M.next.prev = M.prev;
        }
        ...
        M.next =slow; M.prev = slow.prev; slow.prev.next = M; slow.prev =M;
    }
    class Node{
        Integer elem;
        Node next; Node prev;
