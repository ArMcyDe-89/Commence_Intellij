public class QuizOneC {
    public Node reverseClip(Node head, int start, int end){
        ...
        while(N!=null){
            if(count >=start && count<= end){if(rev==null){rev = N;continue;}rev.next = N;continue;}
            N=N.next;count++; if(count>end){last = N; break;}
        }
        rev = InPlaceRev(rev); N =head;
        while(N.next!=null){N=N.next;}N.next=rev; 
        while(N.next!=null){N=N.next;}N.next = last; 
        return rev;
    }
