    ...
    public static void warehouseDataRow(int [][]matrix, Node list1, Node list2){
        ...
        for(int row =0; row<matrix.length;row++){
            ...
            for(int col = 0;col<matrix[row].length;col++){
                if(matrix[row][col]>max){max = matrix[row][col]; num.next.elem = max; pos.next.elem = col+1;}
...
public static void printList(Node head) {
    ...
    public static void main(String[] args) {
        ...
        Node list1_DH = new Node(-1); 
        Node list2_DH = new Node(-1);

        // 3. Run the analysis
        analyzeWarehouse(matrix, list1_DH, list2_DH);
