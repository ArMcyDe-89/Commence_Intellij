...
public class QuizTwoB {
    public Integer faulty(BTNode root, Integer D){
        ...
        int count =0;
        if(root.left!=null && (Math.abs((int)root.elem - (int)root.left.elem)>D)) || (root.right!=null && (Math.abs((int)root.elem - (int)root.right.elem)>D))){count++;}
        else if{count++;}
 ...
 class SensorDriver {
    ...
    private  void printNodeInternal(List<BTNode> nodes, int level, int maxLevel) {
    ...
    private  int maxLevel(BTNode node) {
        if (node == null)
            return 0;
    ...
    public void main(String[] args) {
        ...
        printNode(root1);
        System.out.println("Result: " + faulty(root1, 7));
