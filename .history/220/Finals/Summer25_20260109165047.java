public class Summer25 {
    ...
    public static String helper(BTNode root, int []seq, int i){
        if(root == null || root.elem != seq[i]){return "LOST";}
        if (i == seq.length-1) {
        ...
        String result = "LOST";
        if (root.left!=null && i<seq.length) {
            result = helper(root.left, seq, i+1);
        }
        if (result.equals("LOST") && root.right!= null && i<seq.length) {
            result= helper(root.right, seq, i)
        }
