public class Summer25 {
    ...
    public static String helper(BTNode root, int []seq, int i){
        if(root == null){return "";}if(root.left == null && root.right == null){return "LOST";}
        if (i<seq.length) {
            if(i == seq.length-1){return "ESCAPED";}
            int L = (int)root.left.elem, R = (int)root.right.elem;
            if(L == seq[i+1]){return helper(root.left, seq, i+1);} return helper(root.right, seq, i+1);
        }
        return "TRAPPED";
    }
    static class BTNode {
