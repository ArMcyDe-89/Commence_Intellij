public class Summer25 {
    ...
    public static String helper(BTNode root, int []seq, int i){
        if(root == null){return "";}
        if(i == seq.length-1 && root.left == null && root.right== null){return "ESCAPED";}
        if(i == seq.length-1 && (root.left!= null || root.right!=null)){return "TRAPEED";}
        if(root.left!=null && root.right!=null && i<seq.length-1){
            if(seq[i+1] == root.left.elem){ return  helper(root.left, seq, i+1);}else{ return helper(root.right, seq, i+1);}
        }
        return ""
    }
    static class BTNode {
