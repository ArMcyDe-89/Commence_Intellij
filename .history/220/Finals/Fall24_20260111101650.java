public class Fall24 {
    ...
    static boolean is_complete(Edge[] net){
        ...
        for(int i =0;i<net.length;i++){
            ...
            //for(int j =0; j<met.length;j++){if(met[j] == false){count++;}}if(count>1){return  false;}
        }
    ...
    class HuffmanTree{
        ...
        Character c;
        public HuffmanTree(){parent=null; left = null;right = null;}
        public HuffmanTree(Character C, HuffmanTree Parent,HuffmanTree l, HuffmanTree r){
            c=C; parent=Parent; left = l;right = r;
        }
    }
    static String getEncoding(HuffmanTree root, String word){ // unfinished
        if(root==null){return null;}
        if(root.c!=null){
            for(int i=0;i<word.length();i++){if(root.c == word.charAt(i)){return "";}}
        }
        String left = getEncoding(root.left, word); String right = getEncoding(root.right, word);
        if(left!=null){return "0"+getEncoding(root.left, word);}
        if(right!=null){return "0"+getEncoding(root.right, word);}
        return null;
    }
