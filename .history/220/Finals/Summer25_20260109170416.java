...
// Recursive Helper
    ...
    void urgent_task(int [] tasks, int deadline){
        MaxHeap max = new MaxHeap(tasks.length);
        for(int i=0; i<tasks.length;i++){
            max.insert(tasks[i]);
        }
        int D = deadline; int i =0;
        while(deadline>0){
            
            System.out.printf("Day %d - Urgency %d \n",++i,max.extractMax()); deadline--;
        }
        System.out.println("Tasks Completed: "+D); System.out.println("Tasks Ignored:"+(tasks.length-D));
    }
    class MaxHeap {
