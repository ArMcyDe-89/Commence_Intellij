public class LinkingTheLists {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(checkPalindrome(LinkedList.createList(new Object[]{1 , 2 , 3 , 2 ,1 })));
    }
    public static boolean checkPalindrome(Node head) {
        ...
        while (reach<((count/2)+1)) {;
            M =M.next;reach++;
        }Node start  = M;Node pal = new Node(null);
        while (M!=null) {
            Node put = new Node(M.elem); put.next = pal; pal = put; M = M.next;
        } LinkedList.printLL(pal);
        boolean B = true;
        while (pal.elem!=null) { if(pal.elem == null){ return B;}
            if((int)pal.elem == (int)N.elem){System.out.println("pal "+pal.elem); System.out.println("N "+N.elem);
                pal = pal.next; B =true; continue;
            } B =false;return B;
        }return B;
