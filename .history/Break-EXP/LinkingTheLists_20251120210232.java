    ...
    public static void warehouseDataRow(int [][]matrix, Node list1, Node list2){
        ...
        for(int i = 0; i<matrix.length;i++){
            for(int j =0; j<matrix[i].length-1;j++){
                if(matrix[i][j]>matrix[i][j+1]){shelf = j;max = matrix[i][j];}else if(matrix[i][j+1]>max){shelf=j+1; max = matrix[i][j];}
            }L1.next = new Node (matrix[i][shelf]); L1 = L1.next; L2.next = new Node(shelf+1); L2=L2.next; shelf = -1;
        }
        //To Do
    }
