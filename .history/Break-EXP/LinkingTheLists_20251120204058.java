    ...
    public static void main(String[] args) {
        ...
        swapNodes(LinkedList.createList(new Object[] {1,2,3,4,5}), 2);
            ...
            {103, 122, 58}
                ...
                // Create two dummy-headed linked lists
        LinkedList list1 = new LinkedList(); // max values
        LinkedList list2 = new LinkedList(); // positions

        // Process the warehouse grid
        processWarehouse(matrix, list1, list2);
    }
    public static void warehouseDataRow(int [][]matrix, Node list1, Node list2){
        int shelf = -1; Node L1 = list1, L2 = list2;
        for(int i = 0; i<matrix.length;i++){
            for(int j =0; j<matrix[i].length-1;j++){
                if(matrix[i][j]>matrix[i][j+1]){shelf = j;}else{shelf=j+1;}
            }L1.next = new Node (matrix[i][shelf]); L1 = L1.next; L2.next = new Node(shelf); L2=L2.next; 
        }
        //To Do
    }
