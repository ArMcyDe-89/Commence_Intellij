public class LinkingTheLists {
    ...
    public static Node Intersection(Node head, Node eye) {
        ...
        while (N.next!=null && E.next != null){
            if((int)N.elem == (int)E.elem){//inter =new Node(N.elem); inter = inter.next;}
                inter.elem = N.elem;System.out.println(N.elem);System.out.println(E.elem); inter.next = new Node(null); inter = inter.next;LinkedList.printLL(inter);}
            N = N.next; E= E.next;           
        }leg = inter;
        return inter;        
    }
