public class LinkingTheLists {
    ...
    public static void EvenPartsReversed(Node head){
        Node N = head;
        while(N.next!=null){
            if((int)N.next.elem%2==0){
                while((int)N.next.elem%2!=0){
                    Node back = new Node(null);
                    Node EVE = new Node(N.next.elem);back = EVE; EVE.next = back;LinkedList.printLL(back);
                }
            }
            N=N.next;
        }
