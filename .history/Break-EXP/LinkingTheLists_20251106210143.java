public class LinkingTheLists {
    ...
    public static Node Intersection(Node head, Node eye) {
        Node N = head, E = eye;
        Node inter= new Node(null); Node Sec inter;
        while (N.next!=null && E.next != null){
            if((int)N.elem == (int)E.elem){inter =new Node(N.elem); inter = inter.next;}//inter.elem = (int)N.elem;inter.next = new Node(null); inter = inter.next;LinkedList.printLL(inter);}
            N = N.next; E= E.next;           
        }
        return inter;        
    }
