public class LinkingTheLists {
    ...
    public static void main(String[] args) {
        ...
        buildEvenOddList(M, head, head, M.length);
        Node H2 = new Node(null);
    }
    public static void buildEvenOddList (int [][] M,Node  head1,Node head2, int n)  {
        Node N =  head1;
        while (N.next!=null) { int X = (int)M[((int)N.elem)%n][((int)N.next.elem)];
            if ((X%n)%2 == 0) {
                Node K = new Node(X);K.next = head1.next; head1.next = K;
            }
            else{Node V =N; while (V.next!=null) {V=V.next;}
