public class LinkingTheLists {
    ...
    public static boolean checkPalindrome(Node head) {
        ...
        while (reach<((count/2)+1)) {;
            M =M.next;reach++;
        }Node start  = M;System.out.println("M1 - "+M.elem);Node pal = new Node(null);
        while (M!=null) {
            Node put = new Node(M.elem); put.next = pal; pal = put;
        }
        while (pal.elem!=null) {
            if((int)pal.elem == (int)N.elem){System.out.println(pal.elem);
                pal = pal.next; continue;
            } return false;
        }return true;
