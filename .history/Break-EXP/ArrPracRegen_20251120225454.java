...
public static void  localMinima(int [][] matrix, int threshold){int count =0; String [] found  =new String [(int)Math.pow(matrix.length-1,2)][2];
    for(int i=1; i<matrix.length-1;i++){
        for(int j = 1;j<matrix[i].length-1;j++){ int is  = matrix[i][j];
            if(is<matrix[i][j+1] && is<matrix[i][j-1] && is<matrix[i+1][j] && is<matrix[i-1][j]){
                found[count][count++] = i; found[count][count++] = j; 
            }
        }
    }
    if (count>1) {
