public class LinkingTheLists {
    ...
    public static void main(String[] args) {
        ...
        InPlaceReverseList(pool);
        LinkedList.printLL(reverseClip(pool, 2, 4));

    }
    public static void studentSandwich(int [] st, int [] sa) {
        
    }
    public static Node reverseClip(Node HJ, int start,int end) {
        Node N =HJ,B=HJ, save1=new Node(null), save2 = new Node(null); int count = 1;
        while(N.next!=null){
            if(end-1 == count){save2 = N.next;N.next = null; N=HJ;count=1;break;}
            N=N.next; count++;
        }
        while(N.next!=null){
            if(start-1 == count){save1 = N.next;N.next=null;N=HJ;break;}
            N=N.next; count++;
        }InPlaceReverseList(save1);
        while(N.next!=null){N=N.next;}N.next=save1;
        while(N.next!=null){N=N.next;}N.next=save2;return B;
    }
    public static Node InPlaceReverseList(Node head) {
