public class LinkingTheLists {
    ...
    public static Node EvenPartsReversed(Node head){
        ...
        while(N.next != null){
            if(((int)N.next.elem)%2 == 0){
                Node trav = N.next, save = N, evens = new Node(null); 
                while (((int) trav.elem)%2 == 0) {System.out.println("f"+trav.elem);
                    Node eve  = new Node(trav.elem); eve.next=evens; evens = eve; trav = trav.next; System.out.println("L"+trav.elem); LinkedList.printLL(evens);
                }save.next = evens; 
            }
            N = N.next;
        }
