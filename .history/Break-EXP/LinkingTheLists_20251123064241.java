public class LinkingTheLists {
    ...
    public static Node reverseClip(Node HJ, int start,int end) {
        ...
        while(N.next!=null){System.out.println(N.elem);
            if(count == start-1){save1=N.next;LinkedList.printLL(save2);}
            if(count == end-1){save2 = N.next;N.next=null;N=HJ;count=1; break;} count++; N=N.next;
        }
            Node S = save1;
            while (S.next != null) {
                if(S.next.elem == save2.elem){
                    S.next = null;
                }
            }
