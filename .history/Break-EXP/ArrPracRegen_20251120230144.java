...
public class ArrPracRegen {
    public static void main(String [] args){
        ...
        NeededPlatformsCheck(new String[]{"0900","0940","0950","1100","1500","1800"},
                             new String[]{"0910","1200","1120","1130","1900","2000"});
        NeededPlatformsCheck(new String[]{"0900","1100","1235"},new String[]{"1000","1200","1240"});
        OrderedSubSequence(new int[]{2,6,1,9,4,5,3}); OrderedSubSequence(new int[]{1, 2, 4, 6, 7,3,11,12,13,14,15,16});
        int [][] matrix = {
    {12, 18, 21, 150, 25, 19, 17, 22},
    {14, 200, 20, 23, 160, 21, 18, 19},
    {11, 16, 15, 22, 24, 300, 20, 17},
    {13, 14, 90, 19, 18, 17, 200, 16},
    {15, 13, 12, 14, 250, 15, 14, 13},
    {40, 18, 17, 16, 15, 14, 13, 12},
    {22, 20, 19, 18, 17, 16, 15, 11},
    {30, 25, 22, 20, 19, 18, 40, 10}
};

localMinima(matrix, 100);

}
public static void  localMinima(int [][] matrix, int threshold){int count =0; String [] found  =new String [(int)Math.pow(matrix.length-1,2)];
    for(int i=1; i<matrix.length-1;i++){
        for(int j = 1;j<matrix[i].length-1;j++){ int is  = matrix[i][j];
            if(is<matrix[i][j+1] && is<matrix[i][j-1] && is<matrix[i+1][j] && is<matrix[i-1][j] && is<threshold){
                found[count++] = i+","+j;
            }
        }
    }
    if (count==0) {
        System.out.println("No minima");return;
    }
    if (count>1) {
        System.out.println("the minima - "+found[count-1]); return;
    }System.out.println("the minima -"+found[count-1]);
}
public static void OrderedSubSequence(int [] A){
    ...
    Arrays.sort(C); int start=0,end =0,longest=0;count=0; 
    for(int i=0;i<C.length;i++){
        if(i>0 && Math.abs(C[i]-C[i-1])>1){
            start = end; end = i-1;longest = count;count=0;
        }count++;
    }
...
public static void NeededPlatformsCheck (String [] A, String [] D){//1.12
    int maxPlatforms = 1;
    ...
    //     if (has == false) {
    //         platform++;
    //     }open = occupied.length;System.out.println(Arrays.toString(occupied));

        for (int i = 0; i < arr.length; i++) { //GPT version
        int platformsNeeded = 1; // this train needs at least 1
        for (int j = 0; j < i; j++) {
            if (Integer.parseInt(arr[i]) <Integer.parseInt(dep[j]) && Integer.parseInt(dep[i])>Integer.parseInt(arr[j])) {
                platformsNeeded++;
            }
        }
    ...
    System.out.println("Minimum "+maxPlatforms+" platforms are required to safely arrive and depart all trains.");
    }
