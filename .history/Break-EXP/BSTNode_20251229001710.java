...
class BSTNode {
    ...
    public static BSTNode balanceBST(BSTNode root) {
        int length= countNodes(root); Integer [] A = new Integer[length];
        storeInOrder(root, A, 0);
        return buildBalancedBST(A, 0, length-1);
    }
    public static Integer countNodes(BSTNode root) {
    ...
    // Helper to create a skewed tree (Linked List style)
    public static BSTNode insertSkewed(BSTNode root, int val) {
        if (root == null) return new BSTNode(val);
        root.right = insertSkewed(root.right, val); // Always go right
        return root;
    }

    // Helper to check tree height
    public static int getHeight(BSTNode node) {
        if (node == null) return 0;
        return 1 + Math.max(getHeight(node.left), getHeight(node.right));
    }

    // Helper to verify structure via PreOrder traversal (Root, Left, Right)
