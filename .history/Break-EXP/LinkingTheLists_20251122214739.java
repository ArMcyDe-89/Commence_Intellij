public class LinkingTheLists {
    ...
    public static Node reverseClip(Node HJ, int start,int end) {
        Node N  = HJ, prev = null, later =null,G=prev; int count = 1;
        while(N.next != null){
            if(count>=start && count<=end){
                prev=new Node(N.elem); prev.next = later; later = prev;
                if(count == end){prev = N.next; break;}
            }N=N.next; count++;
        }count =1; N=HJ; LinkedList.printLL(later); LinkedList.printLL(later);
        while(N.next!=null){
            if(count == start-1){N.next = later;} if(count == end-1){N.next = prev;}
            N=N.next; count++;
        }
        // while(N!=null){
        //     if(count == start){
        ...
        //         later = N.next;
        //         N.next = null;
        //         prev = InPlaceReverseList(prev);
        //     }
        //     N=N.next;count++;
        // } N= HJ; count =1;
        // while (count<end) {
        //     G = G.next;
        // }G.next  = later; count=1;
        // while (count<start) {
        //     N = N.next;
        // }N.next = prev; 
        return HJ;
