public class LinkingTheLists {
    ...
    public static void main(String[] args) {
        ...
        Object AV [] = {10,20,30,40,50,60,70,80,90};
        ...
        pool=InPlaceReverseList(pool);
        LinkedList.printLL(pool);
        LinkedList.printLL(reverseClip(pool, 3, 7));
    ...
    public static Node reverseClip(Node HJ, int start,int end) {
        ...
        save1 = InPlaceReverseList(save1);
        while (N.next!=null) {
            N=N.next;
        }N.next =save1;
        while (N.next!=null) {
    ...
    public static Node EvenPartsReversed(Node head){ //2.3 (incomplete)
        Node N = head; Node A = new Node(null),even = null,save = A;; A.next = head;
        while (A.next!=null) {
            if((int)A.next.elem%2==0){
                Node E = A;
                while ((int)E.next.elem%2==0) {
                    Node add  = new Node(E.elem); add.next = even; even = add;
                    E=E.next;
                }A.next=even;while (A.next!=null) {A=A.next;}A.next = E.next;
            }
            A=A.next;
        }
        // while(N.next != null){
