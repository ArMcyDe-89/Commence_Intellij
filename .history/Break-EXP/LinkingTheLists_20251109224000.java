public class LinkingTheLists {
    ...
    public static Node EvenPartsReversed(Node head){
        ...
        // while(N.next!=null){Node save = N;
        //     if(((int)N.next.elem)%2 ==0){ 
        //         Node trav = N.next; ;
        //         while(((int)trav.elem)%2==0 ){
        //             Node eve = new Node(trav.elem); eve.next = evens; evens = eve; trav = trav.next; if (trav ==  null){break;}
        ...
        while(N.next != null){
            ...
            if(((int)N.next.elem)%2 == 0 && N.next!=null){
                ...
                while (((int) trav.elem)%2 == 0 && trav != null) {System.out.println("f"+trav.elem);
                    Node eve  = new Node(trav.elem); eve.next=evens; evens = eve;
                    trav = trav.next; if(trav == null){break;}System.out.println("L"+trav.elem); LinkedList.printLL(evens);
                }save.next = evens; Node s = evens; 
                 while (s.next.elem != null ) {
                     s=s.next;
                 }s.next = trav; if(trav == null){N = trav;} LinkedList.printLL(save); LinkedList.printLL(evens);LinkedList.printLL(trav); 
            }
            LinkedList.printLL(trav); N = N.next;
        }
