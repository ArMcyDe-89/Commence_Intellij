...
class AssgnTask4{
        ...
        public static void rotateSecret( Character[][] board ){
            int i=0,counter=0,j=board[0].length-1, column=board[0].length-1, row =board.length-1, turns= board.length/2, top=0, left=0;
            while (true) {
                while(counter<turns){
                Character save = board[top][column];
            while(j>left){
                board[top][j]=board[top][--j];
            } 
            while (i<row) {
                board[i][left]=board[++i][left];
            }
            while(j<column){
            ...
            }board[top+1][column] = save; counter++;
            }
                turns--;top++;left++;i++;j--;column--;row--; counter=0; if (turns == 0) {break;}
            }
