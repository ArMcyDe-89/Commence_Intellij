public class QuizOneA {
    public void warehouseDataRow(int [][]matrix, Node list1, Node list2){
        Node num = list1, pos = list2;
        for(int row =0; row<matrix.length;row++){
            Integer max = Integer.MIN_VALUE;
            for(int col = 0;col<matrix[row].length;col++){
                if(matrix[row][col]>max){max = matrix[row][col]; num.next = new Node(max, null); pos.next = new Node(col+1, null);}
